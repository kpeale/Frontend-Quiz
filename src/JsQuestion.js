function JsQuestion({ dispatch, quizzes, index, answer }) {
  const hasAnswered = answer !== null;
  const [, , js, a] = quizzes;
  const currentQuestion = js.questions[index];
  const totalQuestions = js.questions.length;
  ...
  return (
    <section className='section__Question'>
      <div className='section__Question-item'>
        ...
        <h3 className='paragraph__secondary'>{js.title}</h3>
      </div>
