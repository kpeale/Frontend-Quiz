function CssQuestions({ dispatch, quizzes, index, answer, points }) {
  const hasAnswered = answer !== null;
  const [, css, ,] = quizzes;
  const currentQuestion = css.questions[index];
  const totalQuestions = css.questions.length;
  ...
  return (
    <section className='section__Question'>
      <div className='section__Question-item'>
        ...
        <h3 className='paragraph__secondary'>{css.title}</h3>
      </div>
